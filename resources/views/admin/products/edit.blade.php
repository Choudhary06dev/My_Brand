...
@section('content')
    ...
    <div class="mb-8">
        ...
        <p class="text-gray-500 mt-1">Update the details for <span class="font-semibold text-gray-700">{{ $product->name }}</span>.</p>
    </div>
    ...
    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-8 pb-12">
        @csrf
        @method('PATCH')
                ...
                <div class="bg-white rounded-2xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] border border-gray-100 p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            ...
                            @if($product->image)
                                <div class="mb-4 relative w-full h-48 rounded-2xl overflow-hidden border border-gray-200 shadow-sm">
                                    <img src="{{ asset('storage/' . $product->image) }}" alt="Current" class="w-full h-full object-cover">
                                ...
                                <div class="mb-4 grid grid-cols-3 gap-2">
                                    @foreach($product->gallery as $img)
                                        <div class="aspect-square rounded-lg overflow-hidden border border-gray-200">
                                            <img src="{{ asset('storage/' . $img) }}" alt="Gallery" class="w-full h-full object-cover">
                                        </div>
                                    @endforeach
                                </div>
                            @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="care_instructions" id="care_instructions" rows="3"
                                class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none"
                                placeholder="e.g. Machine wash warm with like colors...">{{ old('care_instructions', $product->care_instructions) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div>
                            ...
                            <select name="category_id" id="category_id" required
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('category_id', $product->category_id) == $category->id ? 'selected' : '' }} class="font-bold">
                                        {{ $category->name }}
                                    </option>
